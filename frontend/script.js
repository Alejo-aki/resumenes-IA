...
const API_URL = 'https://verbose-goldfish-r46v9x74r7pgf574j-3000.app.github.dev/';
...
async function updateNotebooksDisplay() {
  try {
    ...
    if (!response.ok) {
      throw new Error('Error al obtener los notebooks');
    }
    ...
    console.error('Error:', error);
    notebooksDisplay.innerHTML = "Error al cargar los notebooks.";
  }
}
...
async function openEditor(notebookName, index) {
  try {
    ...
    console.error('Error:', error);
    alert("Error al cargar el contenido para editar. Inténtalo de nuevo.");
  }
}
